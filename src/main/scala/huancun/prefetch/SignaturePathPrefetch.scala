...
class PatternTable(implicit p: Parameters) extends SPPModule {
  ...
  val q = Module(new Queue(chiselTypeOf(io.req.bits), pTableQueueEntries))
  q.io.enq <> io.req
  ...
  val delta_list = readResult.deltaEntries.map(x => Mux((x.cDelta << 2.U) >= readResult.count, 
                                              x.delta, 0.S))
